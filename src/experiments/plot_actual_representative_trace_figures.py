...
def replay_behavior_to_selected_state(
    ...
    """Reconstruct the snapshot represented by a rollout-dataset row."""
    ...
    sampling_mode = str(row.get("state_sampling_mode", "legacy_active_steps"))

    if sampling_mode == "decision_epochs_uniform":
        ...
        for item in snapshots:
            if int(item["behavior_decision_index"]) == target_state_id:
                return copy.deepcopy(item["env_snapshot"])
        raise RuntimeError(
            f"Could not replay decision state_id={target_state_id} for "
            f"scenario={scenario_name}. Collected {len(snapshots)} decision epochs."
        )
    ...
    raise RuntimeError(
        f"Could not replay legacy state_id={target_state_id} for scenario={scenario_name}. "
